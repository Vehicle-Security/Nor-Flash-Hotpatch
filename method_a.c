...
#define VTOR_INDEX_HARDFAULT   (3u)
#define VTOR_INDEX_USAGEFAULT  (6u)
#define THUNK_NOP_HW           (0x0000u)
...
void method_a_init(uint32_t slot_page_addr)
{
    ...
    if (vector_table_flash == NULL) {
        vector_table_flash = (uint32_t *)0x00000000u;
    }

    for (uint32_t i = 0u; i < VTOR_ENTRIES; ++i) {
        g_vtor_ram[i] = vector_table_flash[i];
    }

    /* 覆盖向量表并显式补上 Thumb 位 */
    g_vtor_ram[VTOR_INDEX_HARDFAULT] = ((uint32_t)&MyHardFault) | PC_THUMB_BIT;
    g_vtor_ram[VTOR_INDEX_USAGEFAULT] = ((uint32_t)&MyUsageFault) | PC_THUMB_BIT;

    SCB->VTOR = (uint32_t)g_vtor_ram;
    memory_barrier();

    /* 开启 UsageFault，确保 UDF 指令能进入异常 */
    SCB->SHCSR |= SCB_SHCSR_USGFAULTENA_Msk;
    memory_barrier();

    /* 初始化 thunk 为无害指令 */
    thunk_set_hw(THUNK_NOP_HW); /* movs r0, r0，等价 NOP */
}

/* 确认在 patch 关闭时，执行 slot 只触发一次“无效 fault” */
...
bool measure_method_A(uint32_t page_addr, uint16_t hw, uint32_t N, uint32_t *out_cycles)
{
    ...
    if (!verify_invalid_once(page_addr)) {
        return false;
    }
    ...
    g_patch_enabled = false;
    thunk_set_hw(THUNK_NOP_HW);
    memory_barrier();

    *out_cycles = (cyc_now() - t0);

    /* 每次循环都应在 slot 处 fault 一次，再跳转到 thunk */
    if ((f1 - f0) != N) {
        return false;
    }
    if (N > 0u && g_last_status != HOTFIX_STATUS_PATCH_OK) {
        return false;
    }
