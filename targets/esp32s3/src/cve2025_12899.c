...
typedef int (*handler_fn_t)(icmp_family_t pkt_family,
                            const void *ip_hdr,
...
static void icmp_prepare_case(uint8_t backing[ICMP_BACKING_SIZE], icmp_hdr_t *icmp)
{
    ...
    for (i = 0; i < ICMP_ADJACENT_SIZE; ++i) {
...
static int icmpv6_echo_handler(icmp_family_t pkt_family,
                               const void *ip_hdr,
                               const icmp_hdr_t *icmp,
                               bool verbose)
{
                                   ...
                                   bool verbose)
{
    ...
    for (i = 0u; i < handler_count; ++i) {
        if ((handlers[i].type == icmp->type) &&
            ...
            return handlers[i].handler(pkt_family, ip_hdr, icmp, verbose);
        }
    }
                      ...
                      const cve_target_profile_t *profile)
{
    ...
    if (profile->apply_fix) {
        ret = icmp_call_handlers_fixed(ICMP_FAMILY_IPV4, backing, &icmp, handlers, handler_count, verbose);
        if (ret == PATCH_RESULT_REJECT_ICMP_FAMILY) {
            if (verbose) {
                console_printf(
                    "\r\n[%s] Family-aware dispatch rejected IPv4 -> IPv6 mismatch, ret=%d\r\n",
    ...
    ret = icmp_call_handlers_vuln(ICMP_FAMILY_IPV4, backing, &icmp, handlers, handler_count, verbose);
                                   ...
                                   cve_target_filter_result_t *result)
{
    ...
    icmp_prepare_case(backing, &icmp);
    register_handler(handlers, &handler_count, ICMP_FAMILY_IPV6, 128u, 0u, icmpv6_echo_handler);
    ret = icmp_call_handlers_fixed(ICMP_FAMILY_IPV4, backing, &icmp, handlers, handler_count, false);
