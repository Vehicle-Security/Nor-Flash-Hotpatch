...
extern const uint32_t patch_slot_erase_word;
extern const uint32_t patch_slot_erase_target;
...
{
    return *(const volatile uint32_t *)&patch_slot_erase_word;
}

static uintptr_t patch_slot_addr(void)
{
    return (uintptr_t)&patch_slot_erase_word;
}

static uintptr_t patch_slot_target_addr(void)
{
    return (uintptr_t)&patch_slot_erase_target;
}
...
static void erase_print_patch_status(void)
{
    ...
    board_console_printf("[patch-erase] slot_addr=0x%08lX slot_word=0x%08lX target_word=0x%08lX target_addr=0x%08lX\r\n",
    ...
    if (state == PATCH_SLOT_STATE0) {
        ...
        board_console_write("[patch-erase] semantics: patch disabled by extra clear bit, no 0->1 rewrite\r\n");
    } else {
        board_console_write("[patch-erase] semantics: unrecognized slot word\r\n");
    }
}
