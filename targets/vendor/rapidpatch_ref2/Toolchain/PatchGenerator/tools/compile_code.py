...
def _resolve_tool(explicit_env, fallback_name, windows_default=None):
	env_path = os.environ.get(explicit_env)
	if env_path and os.path.exists(env_path):
		return env_path
	found = shutil.which(fallback_name)
	if found:
		return found
	if windows_default and os.path.exists(windows_default):
...
def compile_code(src):
	...
	clang = _resolve_tool("RAPIDPATCH_CLANG", "clang", r"C:\Program Files\LLVM\bin\clang.exe")
	llc = _resolve_tool("RAPIDPATCH_LLC", "llc", r"C:\Program Files\LLVM\bin\llc.exe")
	if clang is None:
		print("ERROR clang not found. Set RAPIDPATCH_CLANG or add clang to PATH.")
		return
	if llc:
		cmd = f"\"{clang}\" -O2 -emit-llvm -c \"{src}\" -o - | \"{llc}\" -march=bpf -filetype=obj -o \"{CODEO}\""
	else:
		cmd = f"\"{clang}\" -O2 -target bpf -mcpu=v1 -c \"{src}\" -o \"{CODEO}\""
