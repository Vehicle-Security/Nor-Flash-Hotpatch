...
static uint32_t icmp_runtime_pkt_family(const cve_target_input_t *input) {
    return (input != NULL) ? input->runtime_arg0 : (uint32_t)ICMP_FAMILY_IPV4;
}

static uint32_t icmp_runtime_handler_family(const cve_target_input_t *input) {
    return (input != NULL) ? input->runtime_arg1 : (uint32_t)ICMP_FAMILY_IPV6;
}

static uint32_t icmp_runtime_type(const cve_target_input_t *input) {
    return (input != NULL) ? input->runtime_arg2 : 128u;
}

static uint32_t icmp_runtime_code(const cve_target_input_t *input) {
    return (input != NULL) ? input->runtime_arg3 : 0u;
}

static void icmp_prepare_case(uint8_t backing[ICMP_BACKING_SIZE], icmp_hdr_t *icmp, const cve_target_input_t *input) {
    ...
    icmp->type = (uint8_t)icmp_runtime_type(input);
    icmp->code = (uint8_t)icmp_runtime_code(input);
}
...
static int icmpv4_echo_handler(icmp_family_t pkt_family, const void *ip_hdr, const icmp_hdr_t *icmp) {
    const ipv4_hdr_t *hdr = (const ipv4_hdr_t *)ip_hdr;
...
int cve2025_12899_run(const cve_target_input_t *input,
    ...
    icmp_family_t pkt_family = (icmp_family_t)icmp_runtime_pkt_family(input);
    icmp_family_t handler_family = (icmp_family_t)icmp_runtime_handler_family(input);
    ...
    icmp_prepare_case(backing, &icmp, input);
    if (handler_family == ICMP_FAMILY_IPV4) {
        register_handler(handlers, &handler_count, ICMP_FAMILY_IPV4, icmp.type, icmp.code, icmpv4_echo_handler);
    } else {
        register_handler(handlers, &handler_count, ICMP_FAMILY_IPV6, icmp.type, icmp.code, icmpv6_echo_handler);
    }
    ...
    if (profile->apply_fix) {
        ret = icmp_call_handlers_fixed(pkt_family, backing, &icmp, handlers, handler_count);
        if (ret == PATCH_RESULT_REJECT_ICMP_FAMILY) {
            if (verbose) {
                SEGGER_RTT_printf(0,
                    "\r\n[%s] Family-aware dispatch rejected runtime mismatch, ret=%d\r\n",
    ...
    if (verbose) {
        console_puts("\r\n[vuln] dispatching packet with runtime family/type/code.\r\n");
    }

    ret = icmp_call_handlers_vuln(pkt_family, backing, &icmp, handlers, handler_count);
    if (ret == PATCH_RESULT_ATTACK_ICMP_MISDISPATCH) {
        if (verbose) {
            console_puts("  [-] Packet was mis-dispatched into mismatched family handler.\r\n");
